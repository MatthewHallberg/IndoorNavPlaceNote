...
- (void)fillCaptureDeviceResolutions
{
    static NSString* preset[] =
    {
        ...
        @"" // placeholder for AVCaptureSessionPreset3840x2160
    };
    int count = sizeof(preset) / sizeof(preset[0]) - 1;
    if (UnityiOS90orNewer())
    {
        preset[count] = AVCaptureSessionPreset3840x2160; // exists only in iOS >= 9.0
        ++count;
    }
